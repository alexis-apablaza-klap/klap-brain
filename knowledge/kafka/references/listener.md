# Kafka Listener — template `XxxKafkaListener`

Router: `knowledge/kafka/SKILL.md`.

## Cuándo usar

Al implementar el consumer Kafka de un dominio. El listener es la puerta de entrada de mensajes: valida superficialmente, delega toda la lógica de negocio al `XxxProcessor` y confirma el ack. Vive en `dominio/{nombre_dominio}/listener/`.
...
## Anti-patrones específicos de Kafka Listener

- `ack.acknowledge()` al inicio del método, antes de delegar al processor — si el processor falla, el mensaje se pierde igual.
- Capturar `Exception` genérica sin re-throw — los reintentos automáticos de `KafkaConfig` nunca se disparan.
- Lógica de negocio dentro del listener — el listener orquesta, el `Processor` decide (ver skill `processor`).
- No especificar `containerFactory` en `@KafkaListener` — Spring usa un factory genérico sin DLQ ni `AckMode` correcto.
- Envío asincrónico al DLQ manual (sin `.get()`) — si el DLQ falla no queda registro y el mensaje se pierde en silencio.
